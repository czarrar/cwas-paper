...
subdist.name <- "compcor_kvoxs_fwhm08_to_kvoxs_fwhm08"
res <- 4
set_parallel_procs()

outdir <- file.path(base, "nki", "stability", paste("N104", subdist.name, sep="_"))
...
#select.subs <- which(df$all == 2)
#if (length(select.subs) != nsubs) vstop("select.subs not equaling %s, damn", nsubs)

vcat(T, "...reading & maybe filtering distances")
for (i in 1:nscans) {
    sdist.file <- file.path(subdist.dirs[i], "subdist_transform.desc") # this will be the proper distances
