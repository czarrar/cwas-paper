...
ts_list   = op.join(subdir, "%s_%s_ts_scan2_peaks100_2mm.txt" % (scan, strategy))
sca_list  = op.join(subdir, "%s_%s_sca_scan2_peaks100_2mm.txt" % (scan, strategy))
...
sca_files   = joins(sca_dirs, "scan2_peaks100_2mm.nii.gz")
...
for i in range(nsubjects):
    ...
    log_file  = op.join(op.dirname(sca_file), "peaks100_r2z_and_smooth_scan2.log")
    ...
    sfn   = 'qsub_scripts/%s_sca_r2z_and_smooth_scan2_sub%03i.bash' % (scan, i+1)
    sfile = open(sfn, 'w')
