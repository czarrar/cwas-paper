...
#sinkdirs <- file.path(base, sprintf("%s_%s_meanGcor_sink", scans, strategy))
...
df <- ldply(scans, function(scan) {
    sinkdir <- file.path(base, sprintf("%s_%s_meanGcor_sink", scan, strategy))
    rdf <- ldply(rois, function(roi) {
...
write.csv(df, file="/home/data/Projects/CWAS/nki/sca/summarize_sca-iq_meanGcor.csv")
