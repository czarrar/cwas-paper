...
ts.list   <- file.path(subdir, sprintf("%s_%s_ts_scan2_peaks100_2mm.txt", scan, strategy))
...
if (!all(file.exists(ts.files))) stop("not all func masks exist")
...
for (i in 1:length(func.files)) {
    ...
    log.file  <- sprintf('qsub_logs/%s_sca_ts_scan2_sub%03i.log', scan, i)
    ...
    sfn   <- sprintf('qsub_scripts/%s_sca_ts_scan2_sub%03i.bash', scan, i)
    sfile <- "#!/usr/bash\n"
