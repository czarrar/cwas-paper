...
# For smoothing
sigma <- fwhm / 2.3548
op <- sprintf("-kernel gauss %f -fmean", sigma)
...
vcat(T, "strategy: %s; resolution: %i; fwhm: %f", strategy, res, fwhm)
...
for (si in 1:length(scans)) {
  ...
  outfiles <- sapply(infiles, function(infile) {
    ...
    outfile <- paste(prefix, "_smoothed.nii.gz", sep="")
    
    if (file.exists(outfile)) {
        vcat(T, "exists: %s", outfile)
    } else {
        real_cmd <- sprintf(cmd, infile, op, maskfile, outfile)
        cat(real_cmd, "\n")
        system(real_cmd)
    }
  ...
  out_flist <- paste(prefix, "_smoothed.txt", sep="")
