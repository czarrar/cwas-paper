...
func_files <- as.character(read.table(file.path(sbase, "30_compcor_funcpaths_4mm_combined.txt"))[,1])

set_parallel_procs(nforks=1, nthreads=6)
...
#vcat(T, "Global")
#
#func_files <- as.character(read.table(file.path(sbase, "30_global_funcpaths_4mm_combined.txt"))[,1])
#
#
####
## ROI Extraction (Random)
...
#    out_files <- sapply(func_files, function(func_file) {
#        out_file <- file.path(dirname(func_file), paste(roi_base, ".nii.gz", sep=""))
#        if (!file.exists(out_file)) roi_mean_wrapper(func_file, roi_file, mask_file, out_file)
#        return(out_file)
#    })
#    out_files <- unlist(out_files)
#    ofile <- paste("z_global_", roi_base, "_combined.txt", sep="")
#    write.table(out_files, file=ofile, row.names=F, col.names=F)
#}
