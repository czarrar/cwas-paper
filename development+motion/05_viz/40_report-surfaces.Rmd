...
filenames <- list.files(path=vizdir, full.names=T, 
                        pattern="zpics_surface_only_age_[lr]h_(med|lat)")
...
```{r cwas-only-age-surface, fig.width=dims$height, fig.height=dims$width}
# Visualize
...
# Load the images
filenames <- list.files(path=vizdir, full.names=T, 
                        pattern="zpics_surface_age_with_motion_[lr]h_(med|lat)")
images <- lapply(filenames, readJPEG)
...
```{r cwas-age-with-motion-surface, fig.width=dims$height, fig.height=dims$width}
# Visualize
invisible(surfer_montage_viz(images, coords))
```
...
# Load the images
filenames <- list.files(path=vizdir, full.names=T, 
                        pattern="zpics_surface_motion_with_age_[lr]h_(med|lat)")
images <- lapply(filenames, readJPEG)
...
```{r cwas-motion-with-age-surface, fig.width=dims$height, fig.height=dims$width}
# Visualize
invisible(surfer_montage_viz(images, coords))
```
...
```{r glm-cwas-only-age-setup}
# Load the images
filenames <- list.files(path=vizdir, full.names=T, 
                        pattern="zpics_glm_regress_surface_only_age_[lr]h_(med|lat)")
images <- lapply(filenames, readJPEG)
...
```{r glm-cwas-only-age-surface, fig.width=dims$height, fig.height=dims$width}
# Visualize
invisible(surfer_montage_viz(images, coords))
```
...
```{r glm-cwas-age-with-motion-setup}
# Load the images
filenames <- list.files(path=vizdir, full.names=T, 
                     pattern="zpics_glm_regress_surface_age_with_motion_[lr]h_(med|lat)")
images <- lapply(filenames, readJPEG)
...
```{r glm-cwas-age-with-motion-surface, fig.width=dims$height, fig.height=dims$width}
# Visualize
invisible(surfer_montage_viz(images, coords))
```
...
```{r glm-cwas-motion-with-age-setup}
# Load the images
filenames <- list.files(path=vizdir, full.names=T, 
                        pattern="zpics_glm_regress_surface_motion_with_age_[lr]h_(med|lat)")
images <- lapply(filenames, readJPEG)
...
```{r glm-cwas-motion-with-age-surface, fig.width=dims$height, fig.height=dims$width}
# Visualize
invisible(surfer_montage_viz(images, coords))
```
