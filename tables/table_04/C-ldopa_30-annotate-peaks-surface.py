...
table_dir    = "/home2/data/Projects/CWAS/tables/table_04"

# Load df (peaks?)
df_file     = op.join(table_dir, "%s_peaks-volume.csv" % opre)
df          = read_csv(df_file)
df          = df.ix[:,1:]
...
# Abbreviate the Right/Left and cut out BA
df3.Hemi = np.array([ x[0] for x in df3.Hemi ])
df3.BA = np.array([ x[2:] for x in df3.BA ])

# Only have 2-deci for statistic
df3.Statistic = np.round(df3.Statistic, 2)
...
# Save
print "Save"
df3.to_csv(op.join(table_dir, "%s_peaks.csv" % opre))
