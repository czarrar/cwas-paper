...
if (length(args) < 6)
    stop("must have at least 5 arguments: regressors mask rois tvals output-dir")

ev_file <- args[1]
mask_file <- args[2]
roi_file <- args[3]
tval_names <- args[seq(4, length(args)-1, by=2)]
tval_files <- args[seq(5, length(args)-1, by=2)]
