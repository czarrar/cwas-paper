...
suffix      = "cluster_correct_v05_c05/easythresh/thresh_zstat_age.nii.gz"
mdmr_subpaths = {
...
# Output Path
outdir   = op.join(base, "figures/sfig_dev_motion_global")
...
for k,easydir in easydirs.iteritems():
    ...
    if not path.exists(surfdir):
        os.mkdir(surfdir)
    cmd = "./x_vol2surf.py %s/zstat_age.nii.gz %s/thresh_zstat_age.nii.gz %s/surf_thresh_zstat_age" % (easydir, easydir,  surfdir)
    print cmd
    os.system(cmd)
sfiles = [ path.join(easydir, "surfs/surf_thresh_zstat_age") for k,easydir in easydirs.iteritems() ]
all_sfiles = [] 
for sfile in sfiles:
    for hemi in hemis:
