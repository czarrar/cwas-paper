...
clterm      = "cluster_correct_v05_c05"
mdmr_paths  = [
...
for i,study in enumerate(studies):    
    ...
    if not path.exists(surfdir):
        os.mkdir(surfdir)
    ## create it
    cmd = "./x_vol2surf.py %s/logp_%s.nii.gz %s/clust_logp_%s.nii.gz %s/surf_clust_logp_%s" % (easy_paths[i], suffix_names[i], easy_paths[i], suffix_names[i], surfdir, suffix_names[i])
    print cmd
    os.system(cmd)
    ## collate them
    logp_files = { hemi : path.join(surfdir, "surf_clust_logp_%s_%s.nii.gz" % (suffix_names[i], hemi)) for hemi in hemis }
    ## read them
    if study != "adhd200_rerun":
    ...
    for hemi in hemis:
        ...
        # Save
        outprefix = op.join(outdir, "B_%s_surfer_perms" % study)
