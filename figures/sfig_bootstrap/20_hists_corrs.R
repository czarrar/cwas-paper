...
#+ analyze
cor.mat <- cor(pvals.mat, method="s")
df      <- data.frame(r=cor.mat[lower.tri(cor.mat)])
...
ggplot(df, aes(x=r)) + 
    ...
    xlim(0,1) + 
#    ylim(0,6200) + 
    xlab("Spearman Correlation Between Each Pair of Subsamples") + 
...
ggsave(file.path(odir, "B_hist_corrs_scan1.png"), width=4, height=3.5)
