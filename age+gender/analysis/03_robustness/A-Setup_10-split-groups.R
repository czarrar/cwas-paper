# This script splits the FCON1000/INDI sample into two for analyses
# The script also fixes up some stuff

library(plyr)
source("/home/data/Projects/CWAS/share/lib/capitalize.R") ## capitalize and titalize functions

basedir <- "/home/data/Projects/CWAS/share/age+gender/subinfo"
df <- read.csv(file.path(basedir, "03_details.csv"))
...
# Save
write.csv(df, file=file.path(basedir, "03_details_touse.csv"), row.names=F)
...
func.discovery <- file.path(df.discovery$outdir, 
                    "func/bandpass_freqs_0.01.0.1/functional_mni.nii.gz")
write.csv(df.discovery, file=file.path(basedir, "04_discovery_df.csv"), 
            row.names=F)
write.table(func.discovery, file=file.path(basedir, "04_discovery_funcpaths.txt"), 
            row.names=F, col.names=F)

df.replication <- subset(new_df, new_df$sample == "Replication Sample")
func.replication <- file.path(df.replication$outdir, 
                              "func/bandpass_freqs_0.01.0.1/functional_mni.nii.gz")
write.csv(df.replication, file=file.path(basedir, "04_replication_df.csv"), 
          row.names=F)
write.table(func.replication, file=file.path(basedir, "04_replication_funcpaths.txt"), 
            row.names=F, col.names=F)
